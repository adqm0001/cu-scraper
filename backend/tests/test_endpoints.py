...
def test_update_email_success(client, monkeypatch, as_user, async_mock):
    ...
    monkeypatch.setattr(
        "server.get_user_credentials",
        async_mock({"username": "jdoe", "password": "pw", "email": "old@example.com", "is_test": False}),
    )
...
def test_update_password_success(client, monkeypatch, as_user, async_mock):
    ...
    monkeypatch.setattr(
        "server.get_user_credentials",
        async_mock({"username": "jdoe", "password": "pw", "email": "j@example.com", "is_test": False}),
    )
...
def test_update_password_rejects_test_account(client, monkeypatch, as_user, async_mock):
    as_user("1")
    monkeypatch.setattr(
        "server.get_user_credentials",
        async_mock({"username": "jdoe", "password": "pw", "email": "j@example.com", "is_test": True}),
    )
